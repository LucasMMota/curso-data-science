...
grafo4 = graph(edges=c(1,2,2,3,3,4,4,1),directed=F, n=10 )#n=10 vai gerar mais 6 v??rtices isolados
plot(grafo4)
...
V(grafo7)$Peso = c(40,30,30,25) # cria atributos

vertex_attr(grafo7) # le os atributos dos v??rtices

E(grafo7)$TipoAmizade = c("Amigo","Inimigo","Inimigo","Amigo")

edge_attr(grafo7) # le os atributos das arestas

edge_attr(grafo7)$TipoAmizade

vertex_attr(grafo7)$Peso

E(grafo7)$weight = c(1,2,1,3)#definindo um peso para as arestas
...
plot(grafo7, vertex.size=vertex_attr(grafo7)$Peso)#gera grafo com tamanho do V de acordo com o peso
...
plot(grafo7, vertex.size=vertex_attr(grafo7)$Peso, edge.width=edge_attr(grafo7)$weight, vertex.color=vertex_attr(grafo7)$Cor, edge.curved=0.3, frame=T, main="Grafo")
tkplot(grafo7)
